...
def process_sms_sending(alert_type,api_url):
    ...
    with open('reports/' + filename, mode='w', newline='', encoding='utf-8') as csv_file:
        ...
        for  i in range(start_index, end_index + 1):
            ...
            if alert_type =='absentee_alerts':
                message = f"{sms.get('stu_name')} (Id {sms.get('stu_id')}) आज स्कूल में अनुपस्थित है। इस महीने कुल {sms.get('totalA')} अनुपस्थिति हो चुकी हैं। कृपया सुनिश्चित करें कि आपका बच्चा नियमित रूप से स्कूल आए, ताकि शैक्षणिक हानि से बचा जा सके। - {school_name}. Visit https://{school_code}.matrixe.in?pp=1"
            else:
                message = f"{sms.get('fee_due')} Rs school fee is due of {sms.get('stu_name')} (Id {sms.get('stu_id')}). Please pay your child's school fee on time. - {school_name}. Visit https://{school_code}.matrixe.in?pp=1"
